...
def _load_data():
    ...
    Loading the dataset.
    """
    train = [line.strip() for line in open(
        "./data/raw/train.txt", "r").readlines()[1:]]
    raw_x_train = [line.split("\t")[1] for line in train]
    raw_y_train = [line.split("\t")[0] for line in train]

    test = [line.strip()
            for line in open("./data/raw/test.txt", "r").readlines()]
    raw_x_test = [line.split("\t")[1] for line in test]
    raw_y_test = [line.split("\t")[0] for line in test]

    val = [line.strip()
           for line in open("./data/raw/val.txt", "r").readlines()]
...
def _preprocess(raw_x_train, raw_y_train, raw_x_test, raw_y_test, raw_x_val, raw_y_val):
    ...
    """
    print('\n################### Preprocessing Data ###################\n')
    x_train, x_val, x_test, char_index = _tokenize_data(
        raw_x_train, raw_x_test, raw_x_val)
...
def main():
    ...
    _preprocess(raw_x_train, raw_y_train, raw_x_test,
                raw_y_test, raw_x_val, raw_y_val)
