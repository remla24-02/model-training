...
def test_data():
    ...
    for data_type in ["train", "val", "test"]:
        for axis in ["x", "y"]:
            file_path = os.path.join(output_dir, f'preprocessed_{axis}_{data_type}.joblib')
            if os.path.exists(file_path):
                os.remove(file_path)
...
def test_non_determinism_robustness(test_data):
    ...
    params_1 = _get_parameters(random_state=42, epoch=EPOCH, batch_size=BATCH_SIZE)
    accuracy_1 = train_and_evaluate_model(params_1, x_test, y_test)

    params_2 = _get_parameters(random_state=1, epoch=EPOCH, batch_size=BATCH_SIZE)
